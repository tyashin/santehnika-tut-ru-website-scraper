...
class SantechspiderSpider(scrapy.Spider):
    ...
    def parse(self, response: HtmlResponse):
        categories_refs = set(response.xpath(
            "//a[@class='link n-w-navigation-menu__node-link b-zone b-spy-events i-bem n-w-navigation-menu__node-link_level_second']/@href").extract())
        yield from response.follow_all(categories_refs, callback=self.category_parse)

    def category_parse(self, response: HtmlResponse):
        last_page_num = response.xpath("(//div[@class='pagination']/a/@data-page)[last()]").get()
        if last_page_num:
    ...
    def product_parse(self, response: HtmlResponse):
        ...
        iloader.add_xpath('article',
                          "//div[@class='info']//span[contains(text(), 'Артикул:')]/../following-sibling::div[@class='val']/text()")
